...
public class SimpleExpression {
     ...
     * Added the exp button and square root button
     */
    private void computeValue() {
        ...
        if (mOperator.contentEquals("+"))
            ...
            mValue = mOperand1 / mOperand2;
        else if (mOperator.contentEquals("X^Y"))
            mValue = (int) Math.pow(mOperand1, mOperand2);
        else if (mOperator.contentEquals("SQRT"))
            mValue = (int) Math.sqrt(mOperand1);
        else
            mValue = mOperand1 % mOperand2;
    }
